...
let token = process.env.SLACK_API_TOKEN || 'xoxb-205332753795-KmSLgEUaQs4E2EuA1rseNsMP';
...
rtm.on(CLIENT_EVENTS.RTM.AUTHENTICATED, (rtmStartData) => {
    ...
    let afterNoonTime = newDate(now, '16');
    let afterNoonTime5 = newDate(now, '16', '6');
    let intl = setInterval(function () {
        let nowTime = new Date();
        nowTime.setHours(nowTime.getHours() + 7)
        nowTime = nowTime.getTime();
        // console.log('now-------------------------------', new Date(nowTime));
        // console.log('mormning-------------------------------', new Date(noonTime));
        // console.log('mormning-------------------------------', new Date(noonTime5));
